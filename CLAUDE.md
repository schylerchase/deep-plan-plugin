...
Deep-plan composes with four optional/required external tools, plus its own cross-model handoff layer. Behavioral contracts:
...
- **Cross-model handoff (deep-plan export/import)**: `/deep-plan-export-plan` emits portable bundles; `/deep-plan-import-plan` validates, lands, and reviews them in the receiving repo. Provenance is tracked in PLAN frontmatter `routing.handoff_chain` and `.planning/config.json` `_telemetry.handoff`.
...
Doctor command (`/deep-plan-doctor`) verifies all of the above. Required tools fail the install check; optional tools surface as `[INFO]`; handoff chain and telemetry health surface as Tier-2 project check 7 (`[WARN]`-class, never a hard failure).
